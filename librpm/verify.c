...
int rpmVerifyFile_p(const char * root, Header h, int filenum,
		rpmVerifyAttrs * result, rpmVerifyAttrs omitMask)
{
	...
	&&  hge(h, RPMTAG_DIRINDEXES, NULL, (void **) &dirIndexes, NULL))
	{
	    ...
	    *t = '\0';
		...
		t = (char *) stpcpy(t, root);
		while (t > filespec && t[-1] == '/') {
		    --t;
		    *t = '\0';
		}
	    }
	    t = (char *) stpcpy(t, dirNames[dirIndexes[filenum]]);
	    t = (char *) stpcpy(t, baseNames[filenum]);
	}
 ...
 */
static int verifyHeader_p(QVA_t qva, Header h)
	/*@modifies h @*/
{
    HGE_t hge = (HGE_t)headerGetEntryMinMemory;
    char buf[BUFSIZ];
    ...
    *te = '\0';
	...
	rc = rpmVerifyFile_p(prefix, h, i, &verifyResult, omitMask);
	if (rc) {
	    /*@-internalglobs@*/ /* FIX: shrug */
	    if (!(fileAttrs & RPMFILE_MISSINGOK) || rpmIsVerbose()) {
		sprintf(te, _("missing  n %s"), fileNames[i]);
		te += strlen(te);
		ec = rc;
	    }
	    /*@=internalglobs@*/
 ...
 * @return		0 no problems, 1 problems found
 */
static int verifyDependencies_p(rpmdb rpmdb, Header h)
	/*@modifies h @*/
{
	...
	*te = '\0';
	...
	te += strlen(te);
	for (i = 0; i < numConflicts; i++) {
	    if (i) te = (char *) stpcpy(te, ", ");
	    te = (char *) stpcpy(te, conflicts[i].needsName);
	    if (conflicts[i].needsFlags) {
		...
		*te++ = ' ';
		te = (char *) stpcpy(te, conflicts[i].needsVersion);
	    }
	}
...
int showVerifyPackage(QVA_t qva, rpmdb rpmdb, Header h)
{
	...
	if ((rc = verifyDependencies_p(rpmdb, h)) != 0)
	    ec = rc;
    }
    if (qva->qva_flags & VERIFY_FILES) {
	if ((rc = verifyHeader_p(qva, h)) != 0)
	    ec = rc;
    }
...
int rpmVerify_p(QVA_t qva, rpmQVSources source, const char * arg)
{
    ...
    rc = rpmQueryVerify_p(qva, source, arg, rpmdb, showVerifyPackage);
