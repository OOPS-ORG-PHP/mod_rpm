...
int showQueryPackage_p(QVA_t qva, rpmdb rpmdb, Header h)
{
    ...
    *te = '\0';
	...
	te = (char *) stpcpy(te, name);
	te = (char *) stpcpy( stpcpy(te, "-"), version);
	te = (char *) stpcpy( stpcpy(te, "-"), release);
	goto exit;
    }
	...
	te = (char *) stpcpy(te, _("(contains no files)"));
	goto exit;
    }
	...
	if (queryFlags & QUERY_FOR_STATE) {
		...
		case RPMFILE_STATE_NORMAL:
		    te = (char *) stpcpy(te, _("normal        "));
		    break;
		case RPMFILE_STATE_REPLACED:
		    te = (char *) stpcpy(te, _("replaced      "));
		    break;
		case RPMFILE_STATE_NOTINSTALLED:
		    te = (char *) stpcpy(te, _("not installed "));
		    break;
		case RPMFILE_STATE_NETSHARED:
		    te = (char *) stpcpy(te, _("net shared    "));
		    break;
		default:
	...
	// FIX: shrug
	if (!rpmIsVerbose()) {
	    te = (char *) stpcpy(te, dirNames[dirIndexes[i]]);
	    te = (char *) stpcpy(te, baseNames[i]);
	}
	else {
	    char * filespec;
	    int nlink;
...
int showMatches_p(QVA_t qva, rpmdbMatchIterator mi, QVF_t showPackage)
{
    Header h;
    int ec = 0;
...
int rpmQueryVerify_p(QVA_t qva, rpmQVSources source, const char * arg,
...
restart:
    ...
    case RPMQV_SPECFILE:
	if (showPackage != showQueryPackage_p)
	    return 1;
	...
	if (mi == NULL) {
	    ...
	    if (!retcode)
		retcode = showMatches_p(qva, mi, showPackage);
	}
	break;
...
int rpmQuery_p(QVA_t qva, rpmQVSources source, const char * arg)
{
    ...
    rc = rpmQueryVerify_p(qva, source, arg, rpmdb, showQueryPackage_p);
